    ...
    def calc_info(self):
        ...
        if sys.platform=='darwin' and not os.environ.get('ATLAS',None):
            ...
            if get_platform()[-4:] == 'i386' or 'intel' in get_platform() or \
                ...
                intel = 0
            if os.path.exists(os.path.join(sdkroot, 'System/Library/Frameworks/Accelerate.framework')):
                if intel:
                    args.extend(['-msse3'])
                else:
                    args.extend(['-faltivec'])
                link_args.extend(['-Wl,-framework','-Wl,Accelerate'])
            elif os.path.exists(os.path.join(sdkroot, 'System/Library/Frameworks/vecLib.framework')):
                if intel:
                    args.extend(['-msse3'])
                else:
                    args.extend(['-faltivec'])
